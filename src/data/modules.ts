...
export const modules: Module[] = [
  {
    ...
    unitId: "unit-1",
    hook: "Your brain starts guessing before the image is clear.",
    ...
    introCopy: "You'll see a blurry image and try to identify it. Then you'll trace the pathway your brain uses to make sense of incomplete information.",
    traceNodes: [
    ...
    explain: {
      ...
      keyPathway: "Low spatial frequency visual input → OFC (rapid contextual prediction) → Ventral stream → Inferotemporal cortex (detailed object recognition). This pathway shows that perception is not passive. Higher-order areas like the OFC actively predict what you are seeing before complete sensory data is available.",
      whyItMatters: "The OFC's role in generating rapid object predictions from degraded input shows that perception is active construction, not passive reception. The ventral stream and IT cortex pathway is the canonical object recognition route. The OFC shortcut is evidence that cortical feedback shapes what you see before complete information arrives.",
    },
  ...
  },
  {
    ...
    slug: "face-or-not",
    unitId: "unit-1",
    hook: "You keep seeing faces where there aren't any. There's a reason.",
    ...
    introCopy: "You'll view a series of images and decide if each one contains a face. Then you'll see why your brain is so quick to find faces everywhere.",
    traceNodes: [
      { label: "Visual Input", description: "The image enters the visual system, where early processing extracts edges, contrast patterns, and basic shapes. These features could belong to a face or any other object." },
      ...
      { label: "Fusiform Face Area (FFA)", description: "The FFA, a region in the fusiform gyrus with learned expertise for faces, responds strongly to face-like configurations. It is biased to detect faces even in ambiguous or non-face stimuli. This is the neural basis of pareidolia." },
  ...
  },
  {
    ...
    introCopy: "You'll do a short reaction-time task. Some trials include a gentle interruption: a brief visual flash and an optional soft tone. After the trials, you'll trace the brainstem reflex circuit and see how the amygdala can amplify it. Sound is off by default. You can toggle it on if you'd like.",
    traceNodes: [
      { label: "Reflex Path: Sudden Stimulus → Cochlear Root Neurons → PnC → Motor Output", description: "The core acoustic startle circuit is one of the fastest in the nervous system. A sudden stimulus reaches cochlear root neurons, which project directly to the pontine reticular nucleus (PnC). The PnC then drives spinal and facial motor neurons to produce the startle flinch. The whole circuit completes in two to three synapses, before conscious awareness." },
      { label: "Salience Path: Thalamic Input → BLA → CeA → PnC (Amplified Startle)", description: "In parallel, sensory information reaches the basolateral amygdala (BLA) via a fast thalamic relay (the 'low road'). The BLA evaluates salience: not just whether the stimulus is threatening, but whether it matters. If the context is primed, say you're tense or expecting something, the BLA activates the central nucleus (CeA), which feeds back onto the PnC to amplify the startle. This is fear-potentiated startle. The amygdala doesn't create the reflex. It increases the gain on it." },
    ],
    explain: {
      whatHappened: "You performed a reaction-time task under calm and interrupted conditions. On some trials, a brief flash (and optional tone) appeared just before the target. That sudden stimulus was designed to engage your startle reflex, a fast, involuntary brainstem response.",
      whatYourBrainDid: "The sudden stimulus activated a minimal reflex arc: sensory input → cochlear root neurons → pontine reticular nucleus (PnC) → motor neurons. This circuit produces the startle flinch before your cortex registers what happened. At the same time, the amygdala (specifically the basolateral nucleus, BLA) assessed the salience of the event via a fast subcortical route, the 'low road' through the thalamus. The BLA functions as a salience detector, not just a fear center. When it flags the stimulus as significant, it activates the central nucleus (CeA), which amplifies the startle via feedback to the PnC. This is how emotional state or context can make a startle bigger. It's called fear-potentiated startle. The slower cortical 'high road' delivers a detailed, conscious analysis afterward.",
      ...
      whyItMatters: "The acoustic startle reflex is one of the fastest and simplest sensorimotor circuits in the nervous system. Because the amygdala can amplify it, even basic reflexes are modulated by emotional state. The BLA→CeA→PnC pathway is the fear-potentiated startle circuit, and the low-road/high-road distinction explains why your body reacts before your cortex has finished evaluating the stimulus.",
    },
    demoType: "response",
  ...
  },
  {
    ...
    introCopy: "You'll step through a simplified conditioning experiment. First, a neutral cue is paired with an unpleasant outcome, and your brain learns to predict danger. Then the cue appears without the outcome, and the fear response fades. At the end, you'll see whether that fear can come back on its own.",
    traceNodes: [
      { label: "Conditioned Cue (CS)", description: "A neutral stimulus is perceived. After repeated pairing with an unpleasant outcome (the US), the amygdala learns to treat this cue as a danger signal. This is classical fear conditioning." },
    ...
    explain: {
      whatHappened: "A neutral cue (CS) was paired with an unpleasant outcome (US). That's classical fear conditioning. Then the cue appeared alone during extinction, and the fear response faded. After a delay, the cue returned in a test trial. If the fear came back, that was spontaneous recovery.",
      ...
      whyItMatters: "Fear conditioning establishes the amygdala as an associative threat-learning system. Extinction then shows that the brain suppresses fear rather than erasing it. The vmPFC → ITC pathway is central to modern fear research and directly relevant to emotion regulation. Spontaneous recovery proves that conditioned fears persist beneath extinction. As such, exposure therapy has to build strong, context-independent safety memories.",
    },
    demoType: "observation",
  ...
  },
  {
    ...
    traceNodes: [
      ...
      { label: "Top-Down Control Failure Under Overload", description: "As load increases, the dlPFC's capacity is exceeded. Executive control weakens, distractor suppression fails, and irrelevant items begin to intrude. Performance drops. The items weren't erased. The prefrontal system just couldn't maintain them against interference." },
    ],
    explain: {
      whatHappened: "You performed the same memory task under two conditions. Low load: 3 letters, recall immediately. High load: 7 letters, with a math distractor forcing your dlPFC to hold the sequence while processing something else. The task was identical in structure. Only the demand on working memory changed. If your accuracy dropped under high load, that isn't a failure of memory storage. It's a failure of executive control.",
      whatYourBrainDid: "Your dorsolateral prefrontal cortex (dlPFC) actively maintained the letter sequence in working memory. Under low load (3 items, no distraction), the dlPFC managed comfortably. It had spare capacity to suppress interference and keep the items online. Under high load (7 items plus a distractor), the dlPFC was forced to do two expensive things at once: maintain a longer sequence and suppress the competing math task. This executive control is metabolically costly. When demand exceeded capacity, top-down control failed. The dlPFC could no longer keep all items active or filter out interference, and errors increased. This is cognitive overload. It's not a memory problem, but a prefrontal control problem.",
  ...
  },
  {
    ...
    unitId: "unit-5",
    hook: "Your stress response was built to protect you. It doesn't always know when to stop.",
    ...
    introCopy: "You'll assemble the HPA axis in the correct order, from the hypothalamus releasing CRH through cortisol negative feedback. Then you'll see what happens when extreme stress from the central amygdala and PFC prevents the system from shutting down.",
    traceNodes: [
      ...
      { label: "Adrenal Cortex → Cortisol", description: "The adrenal cortex receives ACTH and releases glucocorticoids, primarily cortisol. Cortisol mobilizes energy, suppresses the immune system, and modulates brain function." },
    ...
    explain: {
      whatHappened: "You assembled the HPA axis in sequence, placing each structure and its hormone output in order. The cascade went from hypothalamus through cortisol release, negative feedback, and the override mechanism under extreme stress.",
      whatYourBrainDid: "By building the cascade step by step, you traced the same neuroendocrine pathway your own body uses during stress. The hypothalamus initiates with CRH. The pituitary amplifies with ACTH. The adrenal cortex produces cortisol. Under normal conditions, cortisol feeds back to shut the system down. Under extreme stress, the central amygdala and PFC override that feedback. Cortisol stays elevated, and the chronic damage of allostatic overload follows.",
      ...
      whyItMatters: "The HPA axis is the primary neuroendocrine stress system, and its negative feedback loop is the body's main mechanism for turning the stress response off. When that feedback fails under chronic stress, the consequences are structural: hippocampal atrophy in CA3 and dentate gyrus, PFC dendritic shrinkage, and a measurable shift along the resilience-vulnerability spectrum tracked by vmPFC activation.",
    },
    demoType: "interactive",
  ...
  },
  {
    ...
    explain: {
      ...
      whyItMatters: "Multistable perception is not a quirk of one image — it is a fundamental feature of how the brain resolves ambiguity. Seeing the same mechanism operate across figure–ground assignment, color constancy, and depth reversal reveals the generality of cognitive heuristics. The same automatic selection process extends beyond vision to emotion, empathy, and reasoning.",
    },
    demoType: "interactive",
  ...
  },
  {
    ...
    explain: {
      ...
      whyItMatters: "The prefrontal cortex is not a single monolithic controller — it is a set of specialized subdivisions that collaborate and sometimes compete. Understanding this functional map explains why cognitive depletion has such broad effects: when the PFC is exhausted by fatigue, hunger, or illness, logic, emotion regulation, and contextual prediction all lose capacity simultaneously.",
    },
    demoType: "interactive",
  ...
  },
  {
    ...
    explain: {
      ...
      whyItMatters: "The amygdala is not a simple fear center — it is a salience center that stamps importance onto events. Fear and anxiety both engage it, but through different circuits and with different timing. This distinction matters clinically because anxiety disorders involve overgeneralized threat prediction, not an overactive fear response to present stimuli.",
    },
    demoType: "interactive",
  ...
  },
  {
    ...
    explain: {
      ...
      whyItMatters: "The emotion–arousal–valence framework is foundational to neuroscience. Arousal tells you how activated the body is, valence tells you the direction, and emotion combines both into a transient motivational state. Without this distinction, it is impossible to accurately interpret amygdala function, the fear–anxiety divide, or how raw interoceptive signals become labeled emotional experiences.",
    },
    demoType: "interactive",
  ...
  },
  {
    ...
    shortDescription: "How two learning paradigms produce different kinds of associations — and why fear conditioning is strictly classical.",
    ...
    explain: {
      ...
      whyItMatters: "Fear conditioning is strictly classical — the organism does not choose to become afraid. The CS–US association forms automatically through BLA circuitry, which is why extinction requires active inhibition (vmPFC → ITCs) rather than simple unlearning. The classical–operant distinction also explains why fear can spontaneously return: the original association was never a voluntary choice, so it cannot be voluntarily undone.",
    },
    demoType: "interactive",
