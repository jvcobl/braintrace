...
export const modules: Module[] = [
  ...
  },
  {
    ...
    explain: {
      whatHappened: "You viewed a series of ambiguous images and decided whether each one contained a face. Some were clearly face-like, others were not, and some fell in between. You likely called 'face' on at least some non-face images — that tendency is pareidolia.",
      whatYourBrainDid: "Your fusiform face area (FFA), a patch of cortex in the fusiform gyrus with deep expertise for face processing, responded to each image. The FFA is tuned by years of experience to detect the basic face configuration — two eyes above a nose above a mouth — and it is biased to fire even when this pattern appears in non-face objects. This face-sensitive bias means your brain over-detects faces rather than risk missing one, because faces carry critical social information.",
      keyPathway: "Visual input → Ventral stream → Fusiform Face Area (face-biased expertise) → Face vs. non-face judgment. The FFA's learned specialization for faces drives rapid, automatic detection. When it activates for a non-face stimulus, the result is pareidolia — the compelling illusion of a face in a random pattern.",
      nbb302Connection: "This module connects to NBB302 material on the fusiform face area, face-selective processing, and perceptual expertise. The FFA illustrates how cortical specialization develops through experience. Damage to this region can produce prosopagnosia — an inability to recognize faces despite intact low-level vision — demonstrating that face recognition depends on a dedicated cortical mechanism, not just general object recognition.",
    },
